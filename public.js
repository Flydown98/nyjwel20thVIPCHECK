...
function defaultPublicSeatMeta(){const out=[];let order=1;const vip=new Set(['AL-13','AL-14','AL-15','AR-01','AR-02','AR-03','BL-13','BL-14','BL-15','BR-01','BR-02','BR-03','CL-13','CL-14','CL-15','CR-01','CR-02','CR-03','DL-13','DL-14','DL-15','DR-01','DR-02','DR-03']);const wc=new Set('ABCDEFGHIJKLMNO'.split('').map(row=>`${row}R-15`));const dp=new Set('ABCDEFGHIJKLMNO'.split('').flatMap(row=>[`${row}R-14`,`${row}R-13`]));'ABCDEFGHIJKLMNO'.split('').forEach(row=>['L','R'].forEach(side=>{for(let n=1;n<=15;n++){const code=`${row}${side}-${String(n).padStart(2,'0')}`;out.push({code,row,side,number:n,category:wc.has(code)?'휠체어':dp.has(code)?'장애인지정석':vip.has(code)?'VIP':'일반',enabled:true,wheelchairEligible:wc.has(code),order:order++})}}));return out}
function publicSeatDot(code,m,selected){const cls=['public-seat-dot'];const cat=String(m?.category||'');if(cat.toLowerCase().includes('vip'))cls.push('public-vip');if(cat.includes('장애인지정'))cls.push('public-disabled-priority');if(m?.wheelchairEligible)cls.push('public-wheelchair');if(selected)cls.push('my-seat');return `<span class="${cls.join(' ')}" title="${code}${cat?' · '+cat:''}${selected?' · 내 좌석':''}"></span>`}
